// lib: , url: Nbi

// class id: 1049018, size: 0x8
class :: {
}

// class id: 1534, size: 0xc, field offset: 0x8
class Laa extends Object
    implements Ha {

  const String? jab(Laa) {
    // ** addr: 0xaac25c, size: 0x28
    // 0xaac25c: ldr             x1, [SP]
    // 0xaac260: LoadField: r0 = r1->field_7
    //     0xaac260: ldur            w0, [x1, #7]
    // 0xaac264: DecompressPointer r0
    //     0xaac264: add             x0, x0, HEAP, lsl #32
    // 0xaac268: ret
    //     0xaac268: ret             
  }
}

// class id: 1535, size: 0x18, field offset: 0x8
class Kaa extends Object
    implements Ha {

  const String? jab(Kaa) {
    // ** addr: 0xa9f5dc, size: 0x28
    // 0xa9f5dc: ldr             x1, [SP]
    // 0xa9f5e0: LoadField: r0 = r1->field_b
    //     0xa9f5e0: ldur            w0, [x1, #0xb]
    // 0xa9f5e4: DecompressPointer r0
    //     0xa9f5e4: add             x0, x0, HEAP, lsl #32
    // 0xa9f5e8: ret
    //     0xa9f5e8: ret             
  }
}

// class id: 1536, size: 0x8, field offset: 0x8
abstract class Jaa extends Object {
}

// class id: 1537, size: 0x10, field offset: 0x8
//   const constructor, 
class Iaa extends Object {
}

// class id: 1538, size: 0xc, field offset: 0x8
abstract class Haa<X0> extends Object {
}
