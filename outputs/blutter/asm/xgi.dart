// lib: , url: xgi

// class id: 1049263, size: 0x8
class :: {

  [closure] static mCa? <anonymous closure>(dynamic, dynamic) {
    // ** addr: 0x526a38, size: -0x1
  }
  [closure] static zCa? <anonymous closure>(dynamic, dynamic) {
    // ** addr: 0x526d74, size: -0x1
  }
}

// class id: 732, size: 0x18, field offset: 0x8
class mCa extends Object {

  String? name(mCa) {
    // ** addr: 0xaac25c, size: 0x28
    // 0xaac25c: ldr             x1, [SP]
    // 0xaac260: LoadField: r0 = r1->field_7
    //     0xaac260: ldur            w0, [x1, #7]
    // 0xaac264: DecompressPointer r0
    //     0xaac264: add             x0, x0, HEAP, lsl #32
    // 0xaac268: ret
    //     0xaac268: ret             
  }
  String? LUa(mCa) {
    // ** addr: 0xa9f5dc, size: 0x28
    // 0xa9f5dc: ldr             x1, [SP]
    // 0xa9f5e0: LoadField: r0 = r1->field_b
    //     0xa9f5e0: ldur            w0, [x1, #0xb]
    // 0xa9f5e4: DecompressPointer r0
    //     0xa9f5e4: add             x0, x0, HEAP, lsl #32
    // 0xa9f5e8: ret
    //     0xa9f5e8: ret             
  }
}

// class id: 761, size: 0x1c, field offset: 0x18
class RBa extends dCa {

  List<mCa?> Fk(RBa) {
    // ** addr: 0xaac56c, size: 0x28
    // 0xaac56c: ldr             x1, [SP]
    // 0xaac570: ArrayLoad: r0 = r1[0]  ; List_4
    //     0xaac570: ldur            w0, [x1, #0x17]
    // 0xaac574: DecompressPointer r0
    //     0xaac574: add             x0, x0, HEAP, lsl #32
    // 0xaac578: ret
    //     0xaac578: ret             
  }
}

// class id: 832, size: 0x14, field offset: 0x10
class lCa extends MBa {
}
